...
struct sslda_model {
    ...
    double* log_theta;
    double* log_phi;
    sslda_model(int num_docs_,int num_words_,int num_topics_,int num_labels_, int num_all_words_, sslda_model* init_model=NULL) {
        num_labels = num_labels_;
        num_docs = num_docs_;
        num_topics = num_topics_;
        num_words = num_words_;
    ...
    sslda_model(char* model_root, char* prefix) {
        ...
        mu = load_mat(mu_file, num_topics,1);
    ...
    void init(sslda_model* init_model=NULL);
